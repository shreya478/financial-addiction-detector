...
def train_eval(X_train, y_train, X_test, y_test, target_epsilon):
    ...
    print("Computed Class Weights:", [f"{w:.4f}" for w in class_weights.numpy()])
...
cm, f1s = train_eval(X_train, y_train, X_test, y_test, 10.0)

tiers = ['Casual', 'Frequent', 'Compulsive', 'Crisis']

print("\n--- F1 SCORES PER TIER (Weighted CE) ---")
for i in range(4):
    print(f"{tiers[i]:<15} | {f1s[i]:.4f}")

print("\n--- CONFUSION MATRIX (Weighted CE) ---")
